...
export async function POST(req: Request) {
  ...
  const section = (form.get("section")?.toString() || "").trim().toLowerCase();
  const oldPublicId = (form.get("old_public_id")?.toString() || "").trim();

  if (section !== "hero" && section !== "about") {
    return NextResponse.json({ error: "section debe ser 'hero' o 'about'" }, { status: 400 });
  }
  ...
  const key = `home/${section}/${randomUUID()}.${ext}`;
  ...
  try {
    ...
    console.error("R2 upload home", e);
    return NextResponse.json({ error: "Error subiendo imagen a R2" }, { status: 500 });
  }

  if (oldPublicId && oldPublicId.startsWith("home/")) {
    try {
